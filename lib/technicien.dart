...
class _MyDialogContentState extends State<MyDialogContent> {
  ...
  static List<NominatimResponse> loadPlace(String jsonString) {
    ...
    return parsed
        .map<NominatimResponse>((json) => NominatimResponse.fromJson(json)).toList();
  }
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      content: new Row(
        children: <Widget>[
          Expanded(
//  child: isLoading ? Text(locate,textAlign: TextAlign.center) : new Center(child: CircularProgressIndicator(),
              child: isLoading
                  ? TypeAheadField(
                  ...
                  /*  searchTextField = AutoCompleteTextField<NominatimResponse>(
                    suggestionsAmount: 10,
                      textChanged: (queryString) {
                        if (queryString.length >= 4) getPlace(queryString);
                      },
                      key: key,
                        controller: adresseController,
                      clearOnSubmit: false,
                      ...
                      itemSubmitted: (item) {
                        ...
                        //  searchTextField.textField.controller.text =
                           //   item.displayName;
                           adresseController.text = item.displayName;
                        });
                      },

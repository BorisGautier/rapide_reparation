...
class DetailPage extends StatefulWidget {
  DetailPage(
      {Key key, this.appareil, this.modele, this.probleme, this.ecran,this.pb,this.rdv,this.date})
      : super(key: key);

  final String appareil, modele, probleme, ecran,pb,rdv,date;
  @override
  _DetailPage createState() => _DetailPage(appareil, modele, probleme, ecran,pb,rdv,date);
}

class _DetailPage extends State<DetailPage> {
  _DetailPage(this.appareil, this.modele, this.probleme, this.ecran,this.pb,this.rdv,this.date);
   String appareil, modele, probleme, ecran,pb,rdv,date;
  ...
  confirmer() {
    ...
    if(rdv == "domicile") {
      ...
      setState(() {
        t = token;
        api.reservation(appareil, email, modele, probleme, rdv, date, token).then((Response response){
    ...
    return new Scaffold(
      ...
      body: _isLoading? Center(child:new CircularProgressIndicator())  : ListView(
        children: <Widget>[
          ...
          rd ? Text("NB: l'assistance à domicile vous coutera 20€ deplus",style: TextStyle(fontStyle: FontStyle.italic,color: Colors.red[900]),) : Divider(color: Colors.transparent,height: 5),
                      Center(
                        heightFactor: 1.5,

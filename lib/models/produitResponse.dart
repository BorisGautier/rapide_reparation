...
class ProduitResponse {
  ...
  factory ProduitResponse.fromMap(Map<String, dynamic> json) =>
      new ProduitResponse(
        id: json["id"],
        name: json["name"],
        slug: json["slug"],
        ...
        crossSellIds:
            new List<dynamic>.from(json["cross_sell_ids"].map((x) => x)),
        parentId: json["parent_id"],
        purchaseNote: json["purchase_note"],
        categories: new List<Category>.from(
            json["categories"].map((x) => Category.fromMap(x))),
        tags: new List<Category>.from(
            json["tags"].map((x) => Category.fromMap(x))),
        imageResponses: new List<ImageResponse>.from(
            json["images"].map((x) => ImageResponse.fromMap(x))),
        attributes: new List<Attribute>.from(
            json["attributes"].map((x) => Attribute.fromMap(x))),
        defaultAttributes:
            new List<dynamic>.from(json["default_attributes"].map((x) => x)),
        variations: new List<int>.from(json["variations"].map((x) => x)),
        groupedProducts:
            new List<dynamic>.from(json["grouped_products"].map((x) => x)),
        menuOrder: json["menu_order"],
        metaData: new List<MetaDatum>.from(
  ...
  Map<String, dynamic> toMap() => {
        ...
        "imageResponses":
            new List<dynamic>.from(imageResponses.map((x) => x.toMap())),
        "attributes": new List<dynamic>.from(attributes.map((x) => x.toMap())),
        "default_attributes":
            new List<dynamic>.from(defaultAttributes.map((x) => x)),
        "variations": new List<dynamic>.from(variations.map((x) => x)),
        "grouped_products":
            new List<dynamic>.from(groupedProducts.map((x) => x)),
...
final attributeNameValues = new EnumValues(
    {"Couleurs": AttributeName.COULEURS, "Modèles": AttributeName.MODLES});

enum Backorders { NO }

final backordersValues = new EnumValues({"no": Backorders.NO});

enum CatalogVisibility { VISIBLE }

final catalogVisibilityValues =
    new EnumValues({"visible": CatalogVisibility.VISIBLE});
...
final slugValues = new EnumValues(
    {"iphone-7-plus": Slug.IPHONE_7_PLUS, "reconditionne": Slug.RECONDITIONNE});
...
class Links {
  ...
  factory Links.fromMap(Map<String, dynamic> json) => new Links(
        self: new List<Collection>.from(
            json["self"].map((x) => Collection.fromMap(x))),
        collection: new List<Collection>.from(
...
enum Key {
  ...
  ACCESSORY_IDS,
  SPECIFICATIONS_DISPLAY_ATTRIBUTES,
  SPECIFICATIONS_ATTRIBUTES_TITLE,
  SPECIFICATIONS,
  SLIDE_TEMPLATE,
  WPB_VC_JS_STATUS
}
...
final statusValues = new EnumValues({"publish": Status.PUBLISH});

enum StockStatus { INSTOCK }

final stockStatusValues = new EnumValues({"instock": StockStatus.INSTOCK});

enum TaxStatus { TAXABLE }

final taxStatusValues = new EnumValues({"taxable": TaxStatus.TAXABLE});

enum Type { VARIABLE }

final typeValues = new EnumValues({"variable": Type.VARIABLE});
